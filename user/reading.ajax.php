...
if (strcmp($op, "load") == 0) {
  ...
  $sql = "SELECT device_readings.reading AS reading, device_readings.created_at AS created_at, " .
    ...
    "FROM device_readings, user_dispos " .
    "WHERE device_readings.user_device_id=user_dispos.id AND " .
    "user_dispos.id=:d AND user_dispos.user_id=:uid LIMIT $offset, 10;";
  $stmt = $pdo->prepare($sql);
  $stmt->execute([
    ':d' => $d,
    ':uid' => $userID,
  ...
  $sql = "SELECT device_readings.created_at AS x, device_readings.reading AS y " .
    "FROM device_readings, user_dispos " .
    "WHERE device_readings.user_device_id=user_dispos.id AND " .
    "user_dispos.id=:d AND user_dispos.user_id=:uid ORDER BY device_readings.created_at DESC LIMIT 0, 20;";
  $stmt = $pdo->prepare($sql);
  $stmt->execute([
    ':d' => $d,
    ':uid' => $userID,
