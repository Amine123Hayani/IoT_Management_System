...
if ($_SERVER["REQUEST_METHOD"] == "POST" && count($_FILES) <= 0) {
  ...
  if (isset($_POST["birthDate"])) {
    ...
    if (strlen($birthDate) > 10) {
      array_push($erreurs, VALID_BIRTHDATE_MAX);
    }
  } else {
    $birthDate = null;
  }
  ...
  if (isset($_POST["bio"])) {
    $bio = $_POST["bio"];
    if (strlen($bio) > 1000) {
      array_push($erreurs, VALID_BIO_MAX);
    }
  } else {
    $bio = null;
  }
  ...
  if (count($erreurs)) {
    ...
    if (isset($_POST['gender'])) {
      ...
      if ($K <= 0) {
        array_push($erreurs, VALID_GENDER_WRONG);
      }
    }
    ...
    if (count($erreurs) > 0) {
      ...
      $sql = "UPDATE users SET first_name=:fn, last_name=:ln, gender_id=:gid, " .
        "bio=:b, birthdate=:bd WHERE utulisateur=:un;";
      $stmt = $pdo->prepare($sql);
      $stmt->execute([
        ":fn" => $firstName,
        ":ln" => $lastName,
  ...
  if (isset($_FILES["avatar"])) {
    ...
    if ($size > 2) {
      array_push($erreurs, VALID_AVATAR_MAX);
    } else if (!in_array($extension, ['jpg', 'png', 'jpeg'])) {
      array_push($erreurs, VALID_AVATAR_EXT);
    }
  } else {
    array_push($erreurs, VALID_AVATAR_REQ);
  }
  ...
  if (count($erreurs) > 0) {
    ...
    if (!$isMoved) {
      array_push($erreurs, SOMETHING_WRONG);
      $_SESSION["erreurs"] = $erreurs;
      header("Location: /user/profile.php");
      return;
    }
    ...
    $sql = "INSERT INTO avatars (user_id, name, size, extension) VALUES " .
      "((SELECT id FROM users WHERE utulisateur=:u), :n, :s, :e);";
    $stmt = $pdo->prepare($sql);
    $stmt->execute([
