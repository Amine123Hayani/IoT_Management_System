  ...
  if ($_SERVER['REQUEST_METHOD'] == "POST" && $op == "store") {
    if (isset($_POST['code']) && isset($_POST['reading'])) {
      ...
      $sql = "INSERT INTO device_readings (user_device_id, reading) VALUES " .
        "((SELECT user_dispos.id FROM user_dispos, dispos WHERE " .
        "dispos.id=user_dispos.device_id AND dispos.code=:c), :r);";
      ...
      $sql = "SELECT * FROM device_calibrations WHERE " .
        "user_device_id=(SELECT user_dispos.id FROM user_dispos, dispos WHERE " .
        "dispos.id=user_dispos.device_id AND dispos.code=:c);";
      $stmt = $pdo->prepare($sql);
      $stmt->execute([
        ":c" => $code,
      ]);
    ...
    if (isset($_GET['code'])) {
      ...
      $sql = "SELECT * FROM user_dispos WHERE device_id=(" .
        "SELECT id FROM dispos WHERE code=:c)";
      $stmt = $pdo->prepare($sql);
      $stmt->execute([
        ":c" => $code,
      ]);
