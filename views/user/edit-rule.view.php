      ...
      <div class="col-12">
        <div class="card shadow mt-2">
          <div class="card-body">
            ...
            <form action="/user/rule.php?d=<?php echo $selectedRule['id']; ?>&op=update" method="POST">
                ...
                <div class="col-12 col-md-9">
                  <select name="sensor" id="sensor" class="form-control" required>
                    <?php foreach ($sensors as $sensor) { ?>
                      <option <?php echo ($sensor['id'] == $selectedRule['sensor_id']) ? "selected" : "" ?> value="<?php echo $sensor['id']; ?>">
                ...
                <div class="col-12 col-md-9">
                  <select name="actuator" id="actuator" class="form-control" required>
                    <?php foreach ($actuators as $actuator) { ?>
                      <option <?php echo ($actuator['id'] == $selectedRule['actuator_id']) ? "selected" : "" ?> value="<?php echo $actuator['id']; ?>">
              ...
              <div class="form-group row text-center">
                ...
                <div class="col-12 col-md-9">
                  <input type="text" name="reading" maxlength="5" value="<?php echo isset($selectedRule['value']) ? $selectedRule['value'] : '' ?>" id="reading" class="form-control">
                </div>
              </div>
              <div class="form-group row text-center">
                ...
                <div class="col-12 col-md-9">
                  <select name="operator" id="operator" class="form-control" required>
                    <?php foreach ($operators as $operator) { ?>
                      <option <?php echo ($operator['id'] == $selectedRule['operator_id']) ? "selected" : "" ?> value="<?php echo $operator['id']; ?>">
              ...
              <div class="form-group row text-center">
                ...
                <div class="col-12 col-md-9">
                  <select name="state" id="state" class="form-control" required>
                    <option <?php echo ($selectedRule['state'] == 1) ? "selected" : "" ?> value="1">ON
                    </option>
                    <option <?php echo ($selectedRule['state'] == 0) ? "selected" : "" ?> value="0">OFF
                    </option>
                  </select>
                </div>
              </div>
              <div class="text-center">
                <button id="updateBtn" type="submit" class="btn btn-success">
                  Update Rule
                </button>
              </div>
            </form>
  ...
  <script>
    $(document).ready(function() {
      ...
      updateBtn.click(function(e) {
        ...
        if (sensorValue <= 0) {
          erreurs.push("<?php echo VALID_SENSOR_REQ; ?>");
        }

        if (actuatorValue <= 0) {
          erreurs.push("<?php echo VALID_ACTUATOR_REQ; ?>");
        }

        if (readingValue.length <= 0) {
          erreurs.push("<?php echo VALID_READING_REQ; ?>");
        } else if (readingValue.length > 5) {
          erreurs.push("<?php echo VALID_READING_MAX; ?>");
        } else if (isNaN(parseFloat(readingValue))) {
          erreurs.push("<?php echo VALID_READING_NUM; ?>");
        }

        if (operatorValue <= 0) {
          erreurs.push("<?php echo VALID_OPERATOR_REQ; ?>");
        }

        if (stateValue <= 0) {
          erreurs.push("<?php echo VALID_STATE_REQ; ?>");
        }

        if (erreurss.length > 0) {
          e.preventDefault();
